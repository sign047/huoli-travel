...
var pass = [
  {
    ...
    regTit: "护照号码格式错误",
    regErrTxt:
      "请输入正确的护照号码,中国护照号共9个字符,以G、P、S、D、E开头，后面为阿拉伯数字",
    regExp: /^[E|P|S|G|D]([a-zA-Z]\d{7}|\d{8})$/
  },
  {
    name: "huName",
    errmsg: "请填写姓名",
    regErrTxt: "请输入正确的中文姓名",
    baseTxt: "",
    regExp: /^([\u4E00-\u9FA5])*$/
  },
  ...
  if (arr.length > 0) {
    for (var i = 0, len = arr.length; i < len; i++) {
      ...
      if (!item.createTime || item.createTime == "请选择签证签发时间") {
        showModel({
          title: `请选择${item.visaName}签证签发时间`,
          showCancel: false
        });
        return false;
      }
      if (!item.expireTime || item.expireTime == "请选择签证到期时间") {
        showModel({
          title: `请选择${item.visaName}签证到期时间`,
          showCancel: false
        });
        return false;
      }
