<!--  pic-->
<template>
    <div class="vertical picbox justify-normal">
        <img class="demo" src="http://7xl2r0.com1.z0.glb.clouddn.com/passport/example.png" />
        <div class="vertical justify-normal tips-box">
            <p class="tips">*拍摄如图示的护照首页，请确保漏出四个角</p>
            <p class="tips">*证件为有效证件</p>
            <p class="tips">*扫描角度正对证件, 无倾斜、无抖动</p>
            <p class="tips">*证件无反光且清晰 若灯光过暗，请打开闪光灯或至明亮的地方以便拍摄</p>
            <p class="tips">*网络通畅</p>
        </div>
        <div class="medium next" @tap.stop="showAction" data-index="0">知道了</div>
    </div>
</template>

<script>
export default {
  name: "pic",
  data() {
    return {
      pageType: "passport",
      isPassReplace: false
    };
  },

  components: {},

  computed: {},
  methods: {
    showAction() {}
  }
};
</script>
<style lang='scss' scoped>
/* pages/pic/pic.wxss */
.photo-btn {
  position: absolute;
  top: 50%;
  left: 50%;
  transform: translate3d(-50%, -50%, 0);
}
.picbox {
  height: 100vh;
  background: #000;
}
page {
  background: #f0f0f0;
}
.demo {
  display: block;
  width: 580rpx;
  height: 400rpx;
  margin: 100rpx 0 60rpx;
}
.tips-box {
  width: 580rpx;
}
.tips {
  font-size: 24rpx;
  color: #ababab;
  display: block;
  width: 100%;
  line-height: 36rpx;
}
.next {
  position: absolute;
  bottom: 110rpx;
  left: 50%;
  transform: translatex(-50%);
  width: 520rpx;
  padding: 14rpx 0;
  border: 1px solid #ababab;
  font-size: 30rpx;
  color: #ababab;
  text-align: center;
  border-radius: 6rpx;
  box-shadow: 0 4rpx 8rpx rgba(0, 0, 0, 0.5);
}

/*loading*/
.loadTxt {
  position: absolute;
  left: 50%;
  bottom: 2px;
  transform: translateX(-50%);
  width: 200px;
  text-align: center;
  color: #fff;
  font-size: 38rpx;
}

.mask {
  width: 100%;
  background: rgba(255, 255, 255, 0.1);
  height: 142%;
  position: fixed;
  transition: all ease-in 0.6s;
}

@keyframes lds-spinner {
  0% {
    opacity: 1;
  }
  100% {
    opacity: 0;
  }
}
@-webkit-keyframes lds-spinner {
  0% {
    opacity: 1;
  }
  100% {
    opacity: 0;
  }
}

.lds-css {
  position: absolute;
  z-index: 2;
  top: 28%;
  left: 50%;
  transform: translateX(-50%);
  background: rgba(0, 0, 0, 0.6);
  border-radius: 6px;
  transition: all ease-in 0.6s;
}
.lds-spinner {
  position: relative;
}
.lds-spinner view {
  left: 94px;
  top: 48px;
  position: absolute;
  -webkit-animation: lds-spinner linear 1.7s infinite;
  animation: lds-spinner linear 1.7s infinite;
  background: #fff;
  width: 8px;
  height: 24px;
  border-radius: 40%;
  -webkit-transform-origin: 6px 52px;
  transform-origin: 6px 52px;
}
.lds-spinner view:nth-child(1) {
  -webkit-transform: rotate(0deg);
  transform: rotate(0deg);
  -webkit-animation-delay: -1.558333333333333s;
  animation-delay: -1.558333333333333s;
}
.lds-spinner view:nth-child(2) {
  -webkit-transform: rotate(30deg);
  transform: rotate(30deg);
  -webkit-animation-delay: -1.416666666666667s;
  animation-delay: -1.416666666666667s;
}
.lds-spinner view:nth-child(3) {
  -webkit-transform: rotate(60deg);
  transform: rotate(60deg);
  -webkit-animation-delay: -1.275s;
  animation-delay: -1.275s;
}
.lds-spinner view:nth-child(4) {
  -webkit-transform: rotate(90deg);
  transform: rotate(90deg);
  -webkit-animation-delay: -1.133333333333333s;
  animation-delay: -1.133333333333333s;
}
.lds-spinner view:nth-child(5) {
  -webkit-transform: rotate(120deg);
  transform: rotate(120deg);
  -webkit-animation-delay: -0.991666666666667s;
  animation-delay: -0.991666666666667s;
}
.lds-spinner view:nth-child(6) {
  -webkit-transform: rotate(150deg);
  transform: rotate(150deg);
  -webkit-animation-delay: -0.85s;
  animation-delay: -0.85s;
}
.lds-spinner view:nth-child(7) {
  -webkit-transform: rotate(180deg);
  transform: rotate(180deg);
  -webkit-animation-delay: -0.708333333333333s;
  animation-delay: -0.708333333333333s;
}
.lds-spinner view:nth-child(8) {
  -webkit-transform: rotate(210deg);
  transform: rotate(210deg);
  -webkit-animation-delay: -0.566666666666667s;
  animation-delay: -0.566666666666667s;
}
.lds-spinner view:nth-child(9) {
  -webkit-transform: rotate(240deg);
  transform: rotate(240deg);
  -webkit-animation-delay: -0.425s;
  animation-delay: -0.425s;
}
.lds-spinner view:nth-child(10) {
  -webkit-transform: rotate(270deg);
  transform: rotate(270deg);
  -webkit-animation-delay: -0.283333333333333s;
  animation-delay: -0.283333333333333s;
}
.lds-spinner view:nth-child(11) {
  -webkit-transform: rotate(300deg);
  transform: rotate(300deg);
  -webkit-animation-delay: -0.141666666666667s;
  animation-delay: -0.141666666666667s;
}
.lds-spinner view:nth-child(12) {
  -webkit-transform: rotate(330deg);
  transform: rotate(330deg);
  -webkit-animation-delay: 0s;
  animation-delay: 0s;
}
.lds-spinner {
  width: 139px !important;
  height: 139px !important;
  -webkit-transform: translate(-69.5px, -69.5px) scale(0.695)
    translate(69.5px, 69.5px);
  transform: translate(-69.5px, -69.5px) scale(0.695) translate(69.5px, 69.5px);
}
</style>